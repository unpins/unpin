...
fn print_help_footer(config_path: Option<&std::path::Path>) {
    ...
    println!();
    println!("Networking (opt-in DNS fallback — off by default):");
    println!("  UNPIN_DNS=\"ip ...\"               resolve through these DNS servers when the OS");
    println!(
        "                                   resolver can't be reached (e.g. Android, a captive"
    );
    println!(
        "                                   portal). Space-separated IPv4; a one-shot override"
    );
    println!(
        "                                   of the config `dns` key. Unset → no fallback (the"
    );
    println!(
        "                                   real resolver error is shown, with how to opt in)."
    );
    println!();
    match config_path {
    ...
    println!("    dns          = ip ...      DNS servers for the fallback resolver, used when");
    println!("                               the OS resolver can't be reached (default: off;");
    println!("                               applies to every unpins program, not just unpin)");
}
